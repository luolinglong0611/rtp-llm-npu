"""Device-dispatched implementations of common model operators.

The Ascend path deliberately uses PyTorch operators only.  The legacy Triton
implementations are imported on demand so importing this module is safe in an
NPU-only runtime without Triton installed.
"""
...
def fused_gdn_gating(
    ...
    so its equivalent is expressed with regular PyTorch operations.  Other
    devices continue to use the existing fused Triton kernel.
    """

    if not _is_npu_tensor(a):
        from rtp_llm.models_py.triton_kernels.fla.gdn_gating import (
            fused_gdn_gating as triton_fused_gdn_gating,
        )

        return triton_fused_gdn_gating(
            A_log,
            a,
            b,
            dt_bias,
...
class RmsNormGated(torch.nn.Module):
    ...
    def __init__(
        ...
        assert (
            weight.shape[-1] % self.group_size == 0
        ), "Weight dimension must be divisible by group size, weight_shape: {}, group_size: {}".format(
            weight.shape, self.group_size
        )

    def _get_triton_impl(self):
        if self._triton_impl is None:
            from rtp_llm.models_py.triton_kernels.common.layernorm_gated import (
                RmsNormGated as TritonRmsNormGated,
            )

            self._triton_impl = TritonRmsNormGated(
                self.weight,
                bias=self.bias,
    ...
    def forward(self, x: torch.Tensor, gate: torch.Tensor) -> torch.Tensor:
        ...
        if not _is_npu_tensor(x):
            return self._get_triton_impl()(x, gate)
